...
<form action="admin/category/update/<?php echo $category['id'];?>" method="post" accept-charset="utf-8">
  <div class="con_blk">
    <h3 class="light_gray">分类名称</h3>
    <input name="name" type="text" value="<?php echo $category['name'];?>" class="text">
  </div>
  <div class="con_blk">
      <h3 class="light_gray">缩略名</h3>
      <input name="slug" value="<?php echo $category['slug'];?>" type="text" class="text">
  </div>
  <div class="con_blk">
      ...
      <dl class="set_blk">
        ...
        <select name="pid">
          ...
          <?php foreach($category_list as $val):?>
            <option value="<?php echo $val['id'];?>" <?php if ($val['id'] == $category['pid']):?> selected<?php endif;?>>
              <?php echo $val['tab'];?><?php echo $val['name'];?></option>
          <?php endforeach;?>
          <?php endif;?>
        </select>
        </dd>
  ...
  <div class="con_blk">
      ...
      <textarea name="desc" width="600" height="120"><?php echo $category['desc'];?></textarea>
  </div>
</form>
